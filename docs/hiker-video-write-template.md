...
> 基于河马影视的公共框架提取，标注每个区块「写什么、干什么」，供 AI 写源时直接套用
>
> **文档定位**：这是**模块化框架风格**（顶层 JSON + 子页面模块 + `$.require`），
> 适合**中大型/复杂视频源**（多线路、鉴权、加密、动态选集切换）。
> 轻量视频源（一个 find_rule 走完）建议用《写源模板手册》(doc: blueprint) 的极简模板。
> API 细节以《官方帮助手册》(doc: hiker-help) 为准，本文档的写法均符合官方标准。
...
detail: function () {
  ...
  if (lists.length > 0) {
    ...
    // ⑧a 选集数据编码：统一用 $ 分隔符（标题$videoId$ji$索引，云帧享模式）
...
*海阔视界写源模板 v1 · 基于河马影视公共框架提取 · 供 AI 写源参考*